...
// Add a node with the given handle to the path.
// Not concurrency-safe!
func (n *node) Add(path string, handlers HandlersChain) {
	fullPath := path
	n.priority++
...
walk:
	for {
		...
		if n.IsSplitEdge(i) {
			// 拆分节点
			n.SplitEdge(path, fullPath, i, parentFullPathIndex)
		}

		// 添加子节点？Make new node a child of this node
		if i < len(path) {
			path = path[i:]
			c := path[0]
...
func (n *node) IsSplitEdge(i int) bool {
	return i < len(n.path)
}

func (n *node) SplitEdge(path string, fullPath string, i int, parentFullPathIndex int) {
	child := node{
		path:     n.path[i:],
		nType:    static,
		indices:  n.indices,
...
func (n *node) IsEmpty() bool {
	return len(n.path) == 0 && len(n.children) == 0
}
